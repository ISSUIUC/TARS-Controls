...
def linear_plot(sim_dict):
    """
    Plots 3d position, velocity, and acceleration of the rocket
    Args:
...
def angular_plot(sim_dict):
    ...
    ang_pos_nc.set_ylabel("Angular Position (rad)", fontsize = 10)
    ...
    ang_vel_nc.set_ylabel("Angular Velocity (rad/s)", fontsize = 10)  
    ...
    ang_accel_nc.set_ylabel("Angular Acceleration (rad/s$^2$)", fontsize = 10)   
...
def error_plot(kalman_dict):
    """
    Plots 3d position, velocity, and acceleration of the rocket
    Args:
        kalman_dict: Dictionary containing kalman filter data. Defaults to 0.
    """
...
# Plots a 3d visualization 
def trajactory_plot(sim_dict):
    fig_3d = plt.figure()
    ...
    color_bar = fig_3d.colorbar(plot)
    color_bar.set_label("Velocity Magnitude (m/s)")
    plt.show()
...
def plotter(sim_dict, sensor_dict=0, kalman_dict=0):
    ...
    Args:
        ...
        sensor_dict (dict, optional): Dictionary containing sensor data. Defaults to 0. This is not currently used for now
        kalmann_dict (dict, optional): Dictionary containing kalman filter data. Defaults to 0.
    """
    linear_plot(sim_dict)
    plt.tight_layout()

    angular_plot(sim_dict) 
    plt.tight_layout() 

    error_plot(kalman_dict)
    trajactory_plot(sim_dict)
