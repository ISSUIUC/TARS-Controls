...
class Simulator():
    ...
    def RK4(self, y0, dt, time_stamp, flap_ext=0, staging=False, staging_noise= False, density_noise=False) -> np.ndarray:
        ...
        if staging:
            ...
            ejection_angle = random.gauss(0, .05) if staging_noise else 0
        else:
            ejection_force = 0
            ejection_angle = 0
        ...
        k2_v,k2_av = self.step_v(y0[0], y0[1] + (dt/2)*k1_v, y0[3], y0[4], time_stamp, ejection_force, ejection_angle , flap_ext)
        k3_v,k3_av = self.step_v(y0[0], y0[1] + (dt/2)*k2_v, y0[3], y0[4], time_stamp, ejection_force, ejection_angle, flap_ext)
        k4_v,k4_av = self.step_v(y0[0], y0[1] + dt*k3_v, y0[3], y0[4], time_stamp, ejection_force, ejection_angle, flap_ext)
        ...
        temp,alpha = (self.rocket.forces.get_force(np.array([p, v, y0[3], y0[4]]), flap_ext, time_stamp, ejection_force, ejection_angle, density_noise=density_noise))
    ...
    def step_v(self, pos, vel, ang_pos, ang_vel, time_stamp, ejection_force, ejection_angle, flap_ext):
        ...
        '''
        return self.rocket.forces.get_force(np.array([pos, vel, ang_pos, ang_vel]), flap_ext, time_stamp, ejection_force, ejection_angle)[0] # return slope times mass/inertia 