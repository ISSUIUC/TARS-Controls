...
def addToDict(x, baro_alt, accel, bno_ang_pos, gyro, kalman_filter, alpha, apogee_esimtation, rocket_total_mass, motor_mass, moment_of_inertia):
    ...
    sim_dict["moment_of_inertia_x"].append(moment_of_inertia[0])
    sim_dict["moment_of_inertia_y"].append(moment_of_inertia[1])
    sim_dict["moment_of_inertia_z"].append(moment_of_inertia[2])
...
def simulator(x0, dt) -> None:
    ...
    apogee_estimator = apg.Apogee(kalman_filter.get_state(), 0.1, 0.01, 3, 30, atm) # Use an n value (second to last parameter) that is divisible by 3 to make computations easier
    ...
    while time_stamp < rocket.delay:
        ...
        addToDict(x, baro_alt, accel, bno_ang_pos, gyro, current_state, 0, current_state[0], rocket.rocket_total_mass, rocket.motor_mass, rocket.I_inv(rocket.rocket_total_mass))
    ...
    while x[1, 0] >= 0 or start:
        ...
        addToDict(x, baro_alt, accel, bno_ang_pos, gyro, current_state, alpha, apogee_est, rocket.rocket_total_mass, rocket.motor_mass, rocket.I_inv(rocket.rocket_total_mass))
...
if __name__ == '__main__':
    ...
    for point in range(len(sim_dict["time"])):
        ...
        cur_point += map(str, list([sim_dict["moment_of_inertia_x"][point][0]]))
        cur_point += map(str, list([sim_dict["moment_of_inertia_y"][point][1]]))
        cur_point += map(str, list([sim_dict["moment_of_inertia_z"][point][2]]))
