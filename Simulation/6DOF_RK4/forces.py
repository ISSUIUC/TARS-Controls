...
class Forces:
    ...
    def __init__(self, prop):
        self.prop = prop
        self.motor = m.Motor(prop)
        self.rasaero_file_location = os.path.join(
            os.path.dirname(__file__), prop.rasaero_lookup_file)
    ...
    def get_force(self, x_state, flap_ext, time_stamp) -> np.ndarray:
        ...
        wind_vector = self.atm.get_nominal_wind_direction(
        ) * self.atm.get_nominal_wind_magnitude()
        alpha = self.get_alpha(x_state, wind_vector)
        drag = self.aerodynamic_force(
            x_state, density, wind_vector, alpha, self.rasaero, thrust.dot(thrust) > 0, flap_ext)
        grav = self.gravitational_force(alt, time_stamp)
        force = vct.body_to_world(*x_state[2], thrust + drag) + grav
        moment = vct.body_to_world(
            *x_state[2], np.cross(-self.prop.cm, thrust) + self.aerodynamic_moment(drag))
    ...
    def get_Ca_Cn_Cp(self, x_state, alpha, rasaero, before_burnout, flap_ext) -> list:
        ...
        protub_perc = flap_ext/self.prop.max_ext_length
        ...
        for idx in range(mach_indices[0], mach_indices[-1] + 1):
            if (round(np.degrees(alpha)) == csv_file['Alpha (deg)'][idx] and csv_file['Protuberance (%)'][idx] <= protub_perc <= csv_file['Protuberance (%)'][idx+1]):
                ...
                Ca = np.interp(protub_perc, [
                               csv_file['Protuberance (%)'][idx], csv_file['Protuberance (%)'][idx+1]], [Ca_low, Ca_up])
                Cn = np.interp(protub_perc, [
                               csv_file['Protuberance (%)'][idx], csv_file['Protuberance (%)'][idx+1]], [Cn_low, Cn_up])
                Cp = np.interp(protub_perc, [
                               csv_file['Protuberance (%)'][idx], csv_file['Protuberance (%)'][idx+1]], [Cp_low, Cp_up])
    ...
    def aerodynamic_force(self, x_state, density, wind_vector, alpha, rasaero, before_burnout, flap_ext) -> np.ndarray:
        ...
        '''
        vel = vct.world_to_body(
            *x_state[2].copy(), x_state[1].copy() - wind_vector.copy())
        C_a, C_n, self.prop.cp = self.get_Ca_Cn_Cp(
            x_state, alpha, rasaero, before_burnout, flap_ext)
        roll_aero = np.arctan2(x_state[1, 2], x_state[1, 1])

        # TODO: Check with other values
        C_n_y = np.abs(C_n * np.cos(roll_aero))
        # TODO: Check with other values
        C_n_z = np.abs(C_n * np.sin(roll_aero))
        aero_force = -0.5*np.array([np.sign(vel[0])*vel[0]**2 * C_a*density*self.prop.A,
                                    np.sign(vel[1])*vel[1]**2 *
                                    C_n_y*density*self.prop.A_s,
                                    np.sign(vel[2])*vel[2]**2 * C_n_z*density*self.prop.A_s])
        return aero_force
    ...
    def gravitational_force(self, altitude, time_stamp) -> np.ndarray:
        ...
        '''
        total_mass = self.prop.rocket_dry_mass + \
            self.motor.get_mass(
                time_stamp)  # Adding dry mass + motor mass
        # return np.array([-9.81*total_mass, 0, 0])
        return -np.array([(self.prop.G*self.prop.m_e*total_mass)/((self.prop.r_e+altitude)**2), 0, 0])

    def aerodynamic_moment(self, aerodynamic_force) -> np.ndarray:
        aerodynamic_moment = np.cross(
            self.prop.cp - self.prop.cm, aerodynamic_force)
        return aerodynamic_moment

    def get_alpha(self, x_state, wind_vector) -> float:
        incident_velocity = vct.world_to_body(
            *x_state[2], vct.norm(x_state[1] + wind_vector))
