...
class Forces:
    ...
    def get_force(self, x_state, flap_ext, time_stamp, ejection_force, theta, phi, rocket: rocket_model.Rocket, density_noise=False) -> np.ndarray:
        ...
        grav = self.gravitational_force(alt, time_stamp, rocket)
        force = vct.body_to_world(*x_state[2],thrust + drag) + grav
        moment = vct.body_to_world(*x_state[2], np.cross(-rocket.cm, thrust) + self.aerodynamic_moment(drag, rocket))
        if ejection_force != 0:
            dir = np.array([np.cos(phi), np.sin(phi)* np.sin(theta), np.sin(phi) * np.cos(theta)])
            force += ejection_force * dir
            moment += ejection_force * rocket.stages[rocket.current_stage].cm * dir
        return np.array([force, moment]), alpha

    def get_Ca_Cn_Cp(self, x_state, alpha, rasaero, before_burnout, flap_ext, rocket: rocket_model.Rocket) -> list:
        ...
        alt = x_state[0,0]
        vel = x_state[1]
        mach_number = np.linalg.norm(vel) / rocket.atm.get_speed_of_sound(alt)
        ...
        protub_perc = flap_ext/rocket.max_ext_length
    ...
    def aerodynamic_force(self, x_state, density, wind_vector, alpha, rasaero, before_burnout, flap_ext, rocket: rocket_model.Rocket) -> np.ndarray:
        ...
        C_a,C_n,rocket.cp = self.get_Ca_Cn_Cp(x_state, alpha, rasaero, before_burnout, flap_ext, rocket)
        ...
        aero_force = -0.5*np.array([np.sign(vel[0])*vel[0]**2 * C_a*density*rocket.A, 
                                    np.sign(vel[1])*vel[1]**2 * C_n_y*density*rocket.A_s, 
                                    np.sign(vel[2])*vel[2]**2 * C_n_z*density*rocket.A_s])
        return aero_force
    
    def gravitational_force(self, altitude, time_stamp, rocket: rocket_model.Rocket) -> np.ndarray:
        ...
        '''
        total_mass = rocket.rocket_dry_mass + rocket.motor.get_mass(time_stamp) #Adding dry mass + motor mass

        return -np.array([(prop.G*prop.m_e*total_mass)/((prop.r_e+altitude)**2), 0, 0])

    def aerodynamic_moment(self, aerodynamic_force, rocket: rocket_model.Rocket) -> np.ndarray:
        aerodynamic_moment = np.cross(rocket.cp - rocket.motor.cm, aerodynamic_force)
