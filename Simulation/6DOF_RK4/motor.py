...
class Motor():
    def __init__(self, pr):
        ...
        mass = pr.motor_mass
        delay = pr.delay
        lookup_file: str = pr.motor_lookup_file
        ...
        for i in range(len(self.thrust_data)-1):
            ...
            if float(self.thrust_data["Time (s)"].iloc[i]) < time_stamp and time_stamp < float(self.thrust_data["Time (s)"][i+1]):
                temp = self.lerp_(float(self.thrust_data["Time (s)"].iloc[i]),
                                  float(
                                      self.thrust_data["Time (s)"].iloc[i+1]),
                                  float(
                                      self.thrust_data["Thrust (N)"].iloc[i]),
    ...
    def set_alignment(self) -> None:
        ...
        if self.cur_line < len(self.thrust_data):
            self.alignment = np.array([float(self.thrust_data["Theta"].iloc[self.cur_line]), float(
                self.thrust_data["Phi"].iloc[self.cur_line])])
            self.cur_line += 1
    ...
    def get_mass(self, time_stamp: float) -> np.float64:
        ...
        if time_stamp < self.thrust_data["Time (s)"].iloc[0]:
            ...
            self.current_mass = self.lerp_(float(self.thrust_data["Time (s)"].iloc[0]),
                                           float(
                                               self.thrust_data["Time (s)"].iloc[-1]),
                                           self.total_mass,
                                           0,
                                           time_stamp)
        self.pr.rocket_total_mass = self.pr.rocket_dry_mass + self.current_mass
        self.pr.cm = (self.pr.cm_rocket*self.pr.rocket_dry_mass + self.pr.cm_motor *
                      self.current_mass) / (self.pr.rocket_dry_mass + self.current_mass)
...
if __name__ == '__main__':
    ...
    prop = sp.SimPropeties()
    thrust = np.array([motor.get_mass(t, prop)
                      for t in time], dtype=np.float64)
    # print(thrust.shape)
    # print(time.shape)
    # print(thrust)
