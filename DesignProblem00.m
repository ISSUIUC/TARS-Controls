...
% - EOM
filename = 'DesignProblem00_EOMs.mat';
if (exist(filename,'file')==2)
    fprintf(1,'Loading EOMs from file (delete %s to start fresh).\n',filename);
    load(filename);
else
    [symEOM,numEOM] = GetEOM(process.g,process.m,...
                             process.max_actuation,process.CD_f,...
                             process.CL_f,process.W,...
                             process.CD,process.J,...
                             process.rho, process.D,...
                             process.L, process.Sref);
...
names = {'tStep','g','m','max_actuation', 'CD_f', 'CL_f', 'w', 'CD', 'J', 'rho', 'D', 'L', 'Sref'};
...
function [symEOM,numEOM] = GetEOM(g,m,max_actuation,CD_f, W, CD, J, rho, D, L, Sref)

syms z zdot omega real
x = [z; zdot; omega];
syms l1 l2 real
u = [l1; l2];
syms zddot omegadot
xdot = [zdot; zddot; omegadot];
...
t = get(fig.u.u,'xdata');
x = get(fig.u.u,'ydata');
set(fig.u.u,'xdata',[t process.t],'ydata',[x controller.actuators.l1]);
