...
addParameter(p,'initial',[0;0;0;0]+[1;0.2;0;0].*randn(4,1),...
...
process.processdatatolog = {'t','z','zdot','phi','omega'};
...
% Roll angle
process.phi = process.initial(3,1);
% Rocket roll angular velocity
process.omega = process.initial(4,1);
...
t = process.t;
x = [process.z; process.zdot; process.phi; process.omega];
end

function u = GetInput(process,actuators)
% Copy input from actuators
u = [actuators.l1 ; actuators.l2];

% Bound input
if u(1)<0
    u(1) = 0;
elseif u(1)>process.max_actuation
    u(1) = process.max_actuation;
end

if u(2)<0
    u(2) = 0;
elseif u(2)>process.max_actuation
    u(2) = process.max_actuation;
end
...
d_z = zdot;
d_zdot = -((process.rho*zdot^2)/(2*process.m))*process.Sref*process.CD ...
            - (process.rho*zdot^2*process.CD_f*process.W*(l1 + l2))/process.m ...
            - process.g;
d_phi = omega;
d_omega = (process.rho * zdot^2 * process.CL_f * process.D * process.W * (l1 - l2)) ...
            / (2 * process.J);
% pack xdot
xdot = [d_z; d_zdot; d_phi; d_omega];
end

function iscorrect = CheckActuators(actuators)
iscorrect = false;
if all(isfield(actuators,{'l1','l2'}))&&(length(fieldnames(actuators))==2)
    if isnumeric(actuators.l1) && isnumeric(actuators.l2)
        if isscalar(actuators.l1) && isscalar(actuators.l2)
            iscorrect = true;
        end
    end
end
...
if (isempty(fig))
    ...
    fig.x.omega = plot(nan,nan,'linewidth',2);
    fig.x.ref = plot(nan,nan,':','linewidth',3);
    fig.x.legend = legend({'z','zdot','omega','z_{ref}'});
    xlabel('time');
    
    fig.u.axis = axes('position',[0.55,0.1,0.4,0.35],'fontsize',fs);
    delta = 0.1*process.max_actuation;
    axis([0,process.tStop,0-delta,process.max_actuation+delta]);
    hold on;
    fig.u.l1 = plot(nan,nan,'linewidth',3,'color',[0.0 0.0 0.9]);
    fig.u.l2 = plot(nan,nan,'linewidth',3,'color',[0.9290 0.6940 0.1250]);
    fig.u.umin = plot([0 process.tStop],[0 0],...
                      'linewidth',1,'linestyle','--','color','k');
    fig.u.umax = plot([0 process.tStop],process.max_actuation*[1 1],...
                      'linewidth',1,'linestyle','--','color','k');
    fig.u.legend = legend({'l1','l2'});
    xlabel('time');
...
t = get(fig.x.omega,'xdata');
x = get(fig.x.omega,'ydata');
set(fig.x.omega,'xdata',[t process.t],'ydata',[x process.omega]);

t = get(fig.x.ref,'xdata');
x = get(fig.x.ref,'ydata');
set(fig.x.ref,'xdata',[t process.t],'ydata',[x controller.references.z]);

t = get(fig.u.l1,'xdata');
x = get(fig.u.l1,'ydata');
set(fig.u.l1,'xdata',[t process.t],'ydata',[x controller.actuators.l1]);

t = get(fig.u.l2,'xdata');
...
%     myV = VideoWriter(process.moviefile,'MPEG-4');
    myV = VideoWriter(process.moviefile, 'Motion JPEG AVI');
    myV.Quality = 100;
